...
public class GeneralDisplayView extends Region implements TicketView {
  ...
  private void newServing(String s) {
    String nu = "";
    for (Label label : bigliettiChiamati) {
      String past = label.getText();
      label.setText(nu);
      nu = past;
    }
    bigliettiChiamati.get(0).setText(s);
  }
  ...
  public void set(String s) {
    if (!bigliettiChiamati.get(0).getText().equals(s))
    newServing(s);
  }
}
